...
RequestQueue::Query AddStop(std::istream& is, std::string_view current) {
...
RequestQueue::Query AddBus(std::istream& is, std::string_view current) {
	RequestQueue::Query q;
...
RequestQueue::Query BusInfo(std::istream& is, std::string_view current) {
	RequestQueue::Query q;
	current.remove_prefix(4);//удаляем слово Bus и пробел за ним
...
RequestQueue::Query StopInfo(std::istream& is, std::string_view current) {
	RequestQueue::Query q;
